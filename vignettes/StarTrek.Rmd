---
title: "Working with StarTrek package"
...
- id: ref25
  title: "StarTrek: an R/Bioconductor package for integrative analysis of TCGA data"
  author: 
...
This tool `StarTrek` presents some methodologies to measure pathway activity and cross-talk among pathways integrating also the information of network and TCGA data. New measures are under development.  
...
The following code is used to download all KEGG pathways and genes:

```{r, eval = FALSE}
path<-getKEGGdata(KEGG_path="KEGG_path")
```

The following code is an example to download the pathways involved in energy metabolism:
...
## `get_TCGAdata`: Searching TCGA data for download 
You can easily search TCGA data using the `get_TCGAdata` function.
The TCGA data can be selected using the parameters from `TCGAbiolinks` package. 


```{r, eval = FALSE}
cancer <- "TCGA-BRCA"
PlatformCancer <- "Illumina HiSeq"
tumour<-c("TCGA-BH-A0DL-01A-11R-A115-07")
normal<-c("TCGA-BH-A209-11A-42R-A157-07") 
...
## `path_net`: Interacting genes for each pathway (network data) according a network type (PHint,COloc,GENint,PATH,SHpd)

The function `path_net` creates network data for each pathway. Interacting genes are genes belonging to the same pathway and the interaction is given from network chosen by the user, according the paramenters of the function `getNETdata`.

```{r, eval = FALSE}
netpath<-path_net(pathway=path,net_type=netw)
```
...
```{r, eval = FALSE}
score_mean<-average(dataFilt=Data_CANCER_normUQ_filt,pathway=path)
```

## `score_median`: Median of genes for each pathway starting from a matrix of gene expression 
Starting from a matrix of gene expression (rows are genes and columns are samples, TCGA data) the function `score_median` creates a median matrix of gene expression for each pathway:

```{r, eval = FALSE}
med<-score_median(Data_CANCER_normUQ_filt,path)
```
...
```{r, eval = FALSE}
score_st_dev<-st_dv(dataFilt=Data_CANCER_normUQ_filt,pathway=path)
```
...
## `hamm_dist`: Hamming distance for cross-talk measure
Starting from a matrix of pathway data (rows are genes and columns are pathways) the function `hamm_dist` creates a distance matrix for pairwise pathway based on the common genes among pathways.
...
## `tau_dist`: TAU distance for cross-talk measure
Starting from a matrix of pathway data (rows are values of activity pathway and columns are samples) the function `tau_dist` creates a distance matrix for pairwise pathway based on TAU distance.

```{r, eval = FALSE}
score_tau_dist<-tau_dist(Data_CANCER_normUQ_filt,path)
```
...
```{r, eval = FALSE}
score_euc_dist_crtlkance<-euc_dist_crtlk(dataFilt=Data_CANCER_normUQ_filt,pathway=path)
```

## `dev_std_crtlk`: Sum of standard deviations for cross-talk measure
Starting from a matrix of gene expression (rows are genes and columns are samples, TCGA data) the function `dev_std_crtlk` creates a standard deviation matrix of gene expression for pairwise pathway.
...
nf <- 60
res_class<-svm_classification(TCGA_matrix=score_euc_dist,nfs=nf,normal=colnames(norm),tumour=colnames(tumo))
```
