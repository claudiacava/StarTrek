...
```{r, eval = TRUE}
score_mean<-average(dataFilt=TCGA_matrix,pathway=path)
```
...
```{r, eval = TRUE}
score_st_dev<-st_dv(dataFilt=TCGA_matrix,pathway=path)
```
...
```{r, eval = TRUE}
score_euc_dist_crtlkance<-euc_dist_crtlk(dataFilt=TCGA_matrix,pathway=path)
```
...
This function is used to find the interacting pathway that change in a particular pathology.
