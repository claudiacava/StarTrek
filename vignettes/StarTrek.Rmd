...
```{r, eval = TRUE}
score_mean<-average(dataFilt=tumo[,1:3],pathway=path)
```
...
```{r, eval = TRUE}
med<-score_median(tumo[,1:3],path)
```
...
```{r, eval = TRUE}
score_st_dev<-st_dv(dataFilt=tumo[,1:3],pathway=path)
```
...
```{r, eval = TRUE}
score_tau_dist<-tau_dist(tumo[,1:3],path)
```
...
```{r, eval = TRUE}
cross_talk_st_dv<-dev_std_crtlk(dataFilt=tumo[,1:3],pathway=path)
```
...
```{r, eval = TRUE}
nf <- 60
res_class<-svm_classification(TCGA_matrix=score_euc_dist,nfs=nf,normal=colnames(norm),
                              tumour=colnames(tumo))
